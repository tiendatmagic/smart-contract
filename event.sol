...
contract EventTicketNFT is ERC721, ReentrancyGuard, Ownable {
  ...
  function removeCheckIn(uint256[] memory tIds) external {
  ...
  function getMyTickets(
    address addr,
    uint256 pg
  ) external view returns (TixInfo[] memory, AttInfo[] memory) {
  ...
  function getEventTicketsWithName(
    address usr,
    uint256 eId
  ) external view returns (uint256[] memory, string memory, AttInfo[] memory) {
  ...
  function cancelTicket(uint256[] memory tIds) external {
    for (uint256 i = 0; i < tIds.length; i++) {
      uint256 tId = tIds[i];
      require(_ownerOf(tId) != address(0), "Nonexistent token");
      uint256 eId = tToE[tId];
  ...
  function unCancelTicket(uint256[] memory tIds) external {
    for (uint256 i = 0; i < tIds.length; i++) {
      uint256 tId = tIds[i];
      require(_ownerOf(tId) != address(0), "Nonexistent token");
      uint256 eId = tToE[tId];
  ...
  function withdrawTokenERC721(
    ...
    require(t.ownerOf(tId) == address(this), "Not owner");
    t.safeTransferFrom(address(this), owner(), tId);
  }
  ...
  function getEventsByPage(
    ...
    for (uint256 i = eCnt; i >= 1 && cnt < 10; i--) {
      if (
        ...
        if (++proc > start) {
          (uint256 chkInCnt, uint256 notChkInCnt) = _getEventCheckInStats(i);
          eDets[cnt++] = string(
            abi.encodePacked(
  ...
  function searchEventsByName(
    ...
    for (uint256 i = eCnt; i >= 1 && cnt < 10; i--) {
      if (evs[i].status == status && evs[i].name.toLower().contains(sLow)) {
        if (++proc > start) {
          (uint256 chkInCnt, uint256 notChkInCnt) = _getEventCheckInStats(i);
          eDets[cnt++] = string(
            abi.encodePacked(
  ...
  function recoverTicket(uint256[] memory tIds) external nonReentrant {
    for (uint256 i = 0; i < tIds.length; i++) {
      uint256 tId = tIds[i];
      require(_ownerOf(tId) != address(0));
      uint256 eId = tToE[tId];
      require(evs[eId].creator == msg.sender);
      address own = ownerOf(tId);
      require(own == DEAD || own == address(this));

      uint256[] storage oTix = tByOwn[own];
      for (uint256 j = 0; j < oTix.length; j++) {
        if (oTix[j] == tId) {
          oTix[j] = oTix[oTix.length - 1];
          oTix.pop();
          break;
        }
      }
      ...
      for (uint256 j = 0; j < tByOwn[msg.sender].length; j++) {
        if (tByOwn[msg.sender][j] == tId) {
          alreadyExists = true;
          break;
        }
      }
    ...
    returns (uint256[15] memory)
  {
    ...
    for (uint256 catId = 1; catId <= 15; catId++) {
      ...
      for (uint256 i = 1; i <= eCnt; i++) {
        if (evs[i].id != 0 && evs[i].catId == catId && evs[i].status == 1) {
          tot++;
        }
      }
      tots[catId - 1] = tot;
